...
// graphIsMinute: true=15分足データ / false=日足フォールバック
function BBGraph({ graphCloses, graphBB, graphMA5, graphMA25, currentPrice, isJP, graphIsMinute }) {
  ...
  // ── 全データをそのまま使用（slice廃止） ──────────────────────────────────
  // score.js側で既に適切な本数（15分足:52本程度 / 日足:60本）を渡してくる
  var allCloses = graphCloses;
  var allBB     = graphBB  ? graphBB  : [];
  var allMA5    = graphMA5  ? graphMA5  : [];
  var allMA25   = graphMA25 ? graphMA25 : [];
  var dataLen   = allCloses.length;

  // 価格範囲計算（BB上限/下限・現在値も含む）
  ...
  // 現在値を最右端に追加するため totalLen = dataLen + 1
  ...
  // 現在値縦線のX座標（全データの次 = 最右端）
  var curX = toX(dataLen);
  ...
  return (
    <svg width="100%" viewBox={"0 0 "+W+" "+H} style={{ display:"block", overflow:"visible" }}>
      ...
      {curY !== null && (
        <>
          <line
            x1={curX} y1={PAD.t} x2={curX} y2={PAD.t + innerH}
            stroke="#22d3e0" strokeWidth={1} opacity={0.5} strokeDasharray="3,2" />
          <circle cx={curX} cy={curY} r={3}
            fill="#22d3e0" stroke="#050e1c" strokeWidth={1} />
          <text x={curX + 4} y={curY - 3} fill="#22d3e0"
            style={{ fontSize:7, fontFamily:"monospace" }}>
            {isJP ? Math.round(currentPrice).toLocaleString() : currentPrice.toFixed(2)}
          </text>
        </>
      )}
    ...
    <div style={{ display:"flex", flexDirection:"column", gap:10,
      ...
      <div>
        <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:6 }}>
          <span style={{ fontSize:13, fontWeight:700, color:"#4a90c0" }}>
            📈 BBグラフ
          </span>
          {/* 15分足 or 日足フォールバック バッジ */}
          <span style={{
            fontSize:9, fontWeight:700, padding:"1px 6px", borderRadius:4,
            background: s.graphIsMinute ? "#051a30" : "#1a1000",
            border: "1px solid " + (s.graphIsMinute ? "#22d3e0" : "#fbbf24"),
            color: s.graphIsMinute ? "#22d3e0" : "#fbbf24",
          }}>
            {s.graphIsMinute ? "15分足 過去2日間" : "⚠️ 日足 過去60日"}
          </span>
        </div>
