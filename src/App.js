...
var ACTIVE_TAB_IDS=["fav","trade","event","index","market","news","sync","guide"];
...
// ── 観測ログ（obslog_*）──────────────────────────────────────────────────
// 朝の一瞬しか起きない不具合は、その場でDevToolsを開いていないと後から分からない。
// 在席していない朝でも動きを検証できるよう、イベント・心拍を端末内に貯めておく。
// 記録専用であり、既存の計算・通信内容・画面表示には一切影響させない。
// 書き込みが失敗しても本体を止めないため、読み書きとも例外は全て握りつぶす
var OBS_EVENT_KEY="obslog_event"; // JSエラー・画面の表示切替・日足の取得回数
var OBS_BEAT_KEY="obslog_beat";   // 心拍。アプリが動き続けていた時刻だけを残す
// イベントは件数で切るリングバッファ、心拍だけは件数ではなく直近60分で切る。
// 心拍は30秒間隔なので60分＝約120件になり、100件だと1時間ぶん残らないため
var OBS_EVENT_MAX=100,OBS_BEAT_MIN=60;
...
// 現在の経路は "fetchDaily"（画面全体で使う日足・30分キャッシュ）の1つだけだが、
// 別経路が増えたときに合算して区別できなくなるのを防ぐため src ごとに数える
...
function entryMinutes(e){
  ...
  if(m!=null) return m;
  return (e&&SESSION_MIN[e.session]!=null)?SESSION_MIN[e.session]:null;
}
...
function mergeScanResultDay(date,slots){
  ...
  Object.keys(byTicker).forEach(function(ticker){
    ...
    // 前提にしているため、順序が狂うと的中率が誤る
    hist.sort(function(a,b){
      if(a.d!==b.d) return a.d<b.d?-1:1;
      var ma=entryMinutes(a),mb=entryMinutes(b);
...
function runScanImport(force){
  ...
  if(!force&&st.last&&now-st.last<SCAN_IMPORT_MIN_INTERVAL) return Promise.resolve(false);
  SCAN_IMPORT_BUSY=true;
  var today=jstInfo(0).key,targets=[];
...
function SignalAccuracyContent(p){
  ...
  var impS=useState(0);
  useEffect(function(){
    function onImported(){impS[1](function(v){return v+1;});}
...
function GuidePanel(){
  ...
  var CATS=[
    {key:"fav",icon:"⭐",label:"メイン（お気に入り／全銘柄）",sections:[
      {title:"📋 一覧の使い方",items:[
        "銘柄カードをタップ → 詳細シグナル表示",
        "上部バーは横スクロールします（件数／銘柄検索／グループ／🌱初動順／🏆スコア順／🎯トレード順／🏭業種まとめ登録／📊的中率／再スキャン。PC版では再スキャンが右端に来ます）",
