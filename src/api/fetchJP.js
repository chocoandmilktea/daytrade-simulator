...
// Yahoo Finance 日足データ取得（日本株・15分遅延）
// + BBグラフ用15分足データ追加取得
...
export default async function handler(req, res) {
  ...
  // 日本株ティッカー変換: 3103 → 3103.T
  const cleanCode = code.replace(/\.T$/, "").replace(/0$/, "");
  const ticker = cleanCode + ".T";
  const r = range || "1y";
  ...
  try {
    ...
    if (!resDaily.ok) throw new Error("Yahoo Finance JP daily: " + resDaily.status);

    const dataDaily = await resDaily.json();
    const resultDaily = dataDaily?.chart?.result?.[0];
    if (!resultDaily) throw new Error("empty daily response for " + ticker);
    ...
    const validCloses = closes.filter(function (v) {
