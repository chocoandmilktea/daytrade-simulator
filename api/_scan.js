...
// 組み立て済みマーク（scan:universe:built）のTTLが3日のため、それより長く保つ。
// 本体がマークより先に失効すると「組み立て済み扱いなのに中身が空」になる。
export const UNIVERSE_TTL = 60 * 60 * 24 * 7; // 7日（秒）
...
// 保存はガードを通ったときだけ行う。拒否した場合は理由を返す（呼び出し側でログに出す）。
