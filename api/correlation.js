...
export default async function handler(req, res) {
  ...
  const { ticker, candidates } = req.query;
  if (!ticker) return res.status(400).json({ error: "ticker is required" });
  if (!candidates) return res.status(400).json({ error: "candidates is required" });

  const candidateList = String(candidates)
    .split(",")
  ...
  try {
    ...
    if (!targetReturns || targetReturns.length < MIN_POINTS) {
...
// ── 日次リターン系列の取得（24時間キャッシュ）───────────────────────────
let seriesCache = {}; // { ticker: { ts, returns: number[] } }

async function getReturnSeries(ticker) {
  const now = Date.now();
  if (seriesCache[ticker] && now - seriesCache[ticker].ts < CACHE_TTL) {
    return seriesCache[ticker].returns;
  }
  const closes = ticker.endsWith(".T") ? await fetchJPCloses(ticker) : await fetchUSCloses(ticker);
  const returns = toReturns(closes);
  seriesCache[ticker] = { ts: now, returns: returns };
  return returns;
}
...
// ── JP: J-Quants 日足（code+from+to、stock.jsのbars/minuteと同じ範囲指定方式）──
async function fetchJPCloses(ticker) {
  ...
  return rows.map(function (row) { return row.C || 0; }).filter(function (v) { return v > 0; });
}

// ── US: Yahoo Finance 日足（3ヶ月）──────────────────────────────────────
async function fetchUSCloses(ticker) {
  ...
  return closes.filter(function (v) { return v != null && v > 0; });
}

// ── ピアソン相関係数（系列末尾を基準に長さを揃えて計算）─────────────────
...
// ── JST日付文字列（daysAgo日前、YYYY-MM-DD形式）────────────────────────
