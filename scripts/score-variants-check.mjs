...
// スコアの作り方を変えた場合に、各日の上位10銘柄の成績がどう変わるかを、8:50と10:00の2時点で試算する。
// PR #90・#91（scripts/score-parts-check.mjs・scripts/score-parts-1000-check.mjs）の部品ごとの点数を使い、
// 次の4通りで各日の上位10を選ぶ。
//   S0: 今のスコア（analyzeStock の返すスコアそのまま）
//   S1: 部品の点数の合計から、EXCLUDED_PARTS の9部品と、CAP_PARTS（上限抑制・VIXキャップ）を除いたもの。
//       0〜100への切り詰めはしない
//   S2: S1 から、EXCLUDED_PARTS の9部品の点数の合計を引いたもの
//   R : スコアを使わず、8:50は前日騰落率が低い順、10:00は当日騰落率が低い順
// 部品ごとの点数は analyzeStock の返り値の breakdown（{label, delta} の配列。delta の合計がスコア）から取り出す。
// analyze.js の変更・中身の複製はしない。
//
// 候補の再現・データ取得・2026-06-26 の欠けの扱い・8:50と10:00の時計の差し替え・15分足の切り方・
// 部品ごとの点数の取り出し・売り方の判定・損益AとB・前日騰落率と当日騰落率は
// scripts/score-parts-1000-check.mjs（10:00）と scripts/score-parts-check.mjs（8:50の売買）から写した
// （元のファイルは処理がすべて main() の中にあり、import すると既存のレポートを書き換えるため）。
// 検証日・候補・コスト（往復0.1%）・損益Aの按分（損切りが先2/3）は PR #90・#91 と同じ。
// S0 の上位10の成績は、8:50は PR #88（docs/score-top10-result.md）、10:00は PR #89（docs/score-top10-1000-result.md）の
// レポートと突き合わせる（レポート4章）。
...
//       そこに JSON で保存し、次回以降はそれを読む（形式は PR #88〜#91 のキャッシュと同じ）。
...
// S1 で除き、S2 で差し引く9部品（PR #90・#91 で「すでに上がった銘柄」に点を与える側に出た部品）
var EXCLUDED_PARTS = ["対TOPIX", "トレンド", "EMA整列", "出来高/OBV", "コンフルエンス", "VWAP", "ギャップ", "当日ブレイク", "寄り付きレンジ"];
// S1・S2 で除く、部品の合計にかかる上限（0〜100への切り詰めを含む）
...
var main = async function () {
  ...
  var skip = { noIntraday: 0, noPrevBars: 0, fewBars: 0, noOfficialPrev: 0, noToday: 0, fewBars1000: 0 };
  // 売買から除いた件数（8:50・10:00それぞれ）
  var skipTrade = {
    "0850": { noDaily: 0, noDayBars: 0 },
    "1000": { noDaily: 0, noBuyBar: 0, noAfter: 0 },
  };
  ...
  // 部品の点数から S1・S2 を作る（合計の順番を固定するため PART_LABELS の順に足す）
  var variantScores = function (parts) {
    var kept = 0, excluded = 0;
    PART_LABELS.forEach(function (lb) {
      if (KEPT_PARTS.indexOf(lb) >= 0) kept += parts[lb];
      else if (EXCLUDED_PARTS.indexOf(lb) >= 0) excluded += parts[lb];
    });
    return { s1: kept, s2: kept - excluded };
  };
  ...
  testDays.forEach(function (td) {
    ...
    td.cands.forEach(function (si2, order) {
      var st = stocks[si2];
      // at["0850"] / at["1000"]: その時点のスコアを計算できた場合だけ入る
      var rec = { day: date, ticker: st.ticker, order: order, fromChange: td.byChange.has(si2), at: {} };
      td.records.push(rec);
      var ix = intraday[si2];
      ...
      // 前日騰落率: 前日終値 ÷ 前々日終値 − 1（調整後終値。候補の値上がり率と同じ値。scripts/score-parts-check.mjs と同じ）
      var c1 = bars[si2][t - 1], c2 = bars[si2][t - 2];
      var e = rec.at["0850"] = {
        s0: r0.a.save.score, s1: v0.s1, s2: v0.s2,
        rise: c1 && c2 && isNum(c1.adj) && isNum(c2.adj) && c2.adj > 0 ? c1.adj / c2.adj - 1 : null,
      };
      // 売買: T当日の日足の始値で買い、T当日の15分足で利確・損切りを判定する（PR #88・#90 と同じ）
      if (!dailyOk(b)) {
        skipTrade["0850"].noDaily++;
      } else {
        var dayBars = barsFrom(ix, date, -Infinity);
        if (!dayBars.length) {
          skipTrade["0850"].noDayBars++;
        } else {
          e.exit = judgeExit(b.open, b.close, dayBars);
          var oe = outcome(e.exit, "prorata");
          e.retA = oe.ret;
          e.winA = oe.win;
      ...
      // 10:00: PR #89・#91 と同じ計算。公式の前日終値は T当日の前日の日足終値
      ...
      // 当日騰落率: 10:00時点の現在値 ÷ 前日の日足終値 − 1（scripts/score-parts-1000-check.mjs と同じ）
      var m = rec.at["1000"] = {
        s0: r1.a.save.score, s1: v1.s1, s2: v1.s2,
        rise: isNum(prevIntra) && prevIntra > 0 && isNum(price) ? price / prevIntra - 1 : null,
      };
      // 売買: PR #89・#91 と同じ除外条件と買値（10:00開始の足の始値）
      if (!dailyOk(b)) { skipTrade["1000"].noDaily++; return; }
      var buy = openAt(ix, date, EVAL.buy);
      if (buy == null) { skipTrade["1000"].noBuyBar++; return; }
      var after = barsFrom(ix, date, jstAt(date, EVAL.buy) / 1000);
      if (!after.length) { skipTrade["1000"].noAfter++; return; }
      m.exit = judgeExit(buy, b.close, after);
      var om = outcome(m.exit, "prorata");
      m.retA = om.ret;
      m.winA = om.win;
  ...
  var SLOTS = [
    { key: "0850", label: "8:50", riseLabel: "前日騰落率", buyNote: "T当日の日足の始値で買う（PR #88・#90 と同じ）" },
    { key: "1000", label: "10:00", riseLabel: "当日騰落率", buyNote: "10:00開始の15分足の始値で買う（PR #89・#91 と同じ）" },
  ];
  var METHODS = [
    { key: "s0", label: "S0" },
    { key: "s1", label: "S1" },
  ...
  // 各日の並べ替え。対象はその時点のスコアを計算できた候補（PR #88・#89 の順位付けと同じ母集団）。
  // S0・S1・S2 は値の高い順、R は騰落率の低い順。同点は候補の並び順（order）が先のものを上にする（PR #88 と同じ）。
  // R で騰落率を計算できなかった候補は最後に回す
  var rankKey = function (x, method) {
    if (method === "r") return isNum(x.rise) ? -x.rise : null;
    return x[method];
  };
  var rankDay = function (td, slot, method) {
    var list = td.records.filter(function (r) { return r.at[slot]; });
    list.sort(function (a, b) {
      var ka = rankKey(a.at[slot], method), kb = rankKey(b.at[slot], method);
      var na = !isNum(ka), nb = !isNum(kb);
      if (na || nb) return na === nb ? a.order - b.order : (na ? 1 : -1);
      return kb - ka || a.order - b.order;
    });
    return { list: list, top: list.slice(0, TOP_N), rest: list.slice(TOP_N) };
  };
  var traded = function (list, slot) { return list.filter(function (r) { return r.at[slot].exit; }); };
  var avgAt = function (list, slot, f) {
    var v = list.map(function (r) { return r.at[slot][f]; }).filter(isNum);
  ...
  // PR #88・#89 のレポートと同じ形の行（件数・平均損益率・勝率・利確・損切り・大引け）
  var aggRow = function (label, list, slot, mode) {
    var tr = traded(list, slot);
    if (!tr.length) return "| " + label + " | 0 | - | - | - | - | - |";
    var o = tr.map(function (r) { return outcome(r.at[slot].exit, mode); });
    var sum = function (f) { return o.reduce(function (s, x) { return s + x[f]; }, 0); };
    var n = tr.length;
    return "| " + label + " | " + n + " | " + pct3(sum("ret") / n) + " | " + pct1(sum("win") / n) + " | " + pct1(sum("tp") / n) + " | " + pct1(sum("sl") / n) + " | " + pct1(sum("cl") / n) + " |";
  };
  ...
  SLOTS.forEach(function (sl) {
    var byMethod = {};
    var all = [], riseMissing = 0;
    testDays.forEach(function (td) {
      td.records.forEach(function (r) {
        if (!r.at[sl.key]) return;
        all.push(r);
        if (!isNum(r.at[sl.key].rise)) riseMissing++;
      });
    });
    METHODS.forEach(function (me) {
      var top = [], perDay = [];
      testDays.forEach(function (td) {
        var rk = rankDay(td, sl.key, me.key);
        rk.top.forEach(function (r) { top.push(r); });
        var tt = traded(rk.top, sl.key), trr = traded(rk.rest, sl.key);
        perDay.push({
          date: td.date,
          dA: tt.length && trr.length ? avgAt(tt, sl.key, "retA") - avgAt(trr, sl.key, "retA") : null,
          dB: tt.length && trr.length ? avgAt(tt, sl.key, "retB") - avgAt(trr, sl.key, "retB") : null,
        });
      });
      byMethod[me.key] = { top: top, perDay: perDay };
    });
    result[sl.key] = { all: all, riseMissing: riseMissing, byMethod: byMethod };
  });

  // ---------- S0 の上位10と PR #88・#89 のレポートの突き合わせ ----------

  // レポートの見出し secPrefix（"## 2." など）の中で、modeIdx 番目の「同じ15分足で両方に届いた場合」の表の label の行
  var reportRow = function (path, secPrefix, modeIdx, label) {
    var lines = readFileSync(fileURLToPath(new URL(path, import.meta.url)), "utf8").split("\n");
    var seen = -1, inSec = false;
    for (var i = 0; i < lines.length; i++) {
      if (lines[i].indexOf("## ") === 0) inSec = lines[i].indexOf(secPrefix) === 0;
      if (!inSec) continue;
  ...
  var PRORATA_IDX = 2;
  var CHECKS = [
    { slot: "0850", pr: "PR #88", path: PR88_REPORT, sec: "## 2.", label: "スコア上位10", expectN: 355, expectAvg: "-0.032%" },
    { slot: "1000", pr: "PR #89", path: PR89_REPORT, sec: "## 3.", label: "10:00のスコアで上位10", expectN: 357, expectAvg: "-0.114%" },
  ];
  var cellsOf = function (row) { return row ? row.split("|").map(function (c) { return c.trim(); }).filter(Boolean) : null; };
  CHECKS.forEach(function (c) {
    c.ours = aggRow(c.label, result[c.slot].byMethod.s0.top, c.slot, MODES[PRORATA_IDX].key);
    c.theirs = reportRow(c.path, c.sec, PRORATA_IDX, c.label);
    var oc = cellsOf(c.ours);
    c.nAvgOk = oc[1] === String(c.expectN) && oc[2] === c.expectAvg;
    c.rowOk = c.ours === c.theirs;
  });

  // ---------- 出力 ----------

  var L = [];
  L.push("# スコアの作り方を変えた場合の上位10の成績 試算結果");
  L.push("");
  L.push("- 生成: `node scripts/score-variants-check.mjs`（実行日 " + new RealDate().toISOString().slice(0, 10) + "）");
  L.push("- 検証日・候補・15分足の切り方・スコアの計算・売り方の判定: `scripts/score-parts-check.mjs`（PR #90、8:50）と `scripts/score-parts-1000-check.mjs`（PR #91、10:00）と同じ。候補は8:50時点のもの（前日の確定値で、出来高上位" + NORMAL_VOL_TOP + "と、値上がり率上位" + CHANGE_TOP + "（出来高が全銘柄の中央値の" + VOL_MULT + "倍以上のもの）を重複なしで並べたもの）で、10:00も同じ候補を使う");
  L.push("- 銘柄一覧: JPX 東証上場銘柄一覧（" + (jpx.asOf || "日付不明") + " 時点）のうち、プライム・スタンダード・グロースの内国株式 " + jpx.list.length + "銘柄");
  L.push("- 15分足: Yahoo Finance（interval=" + INTRADAY_INTERVAL + "、range=" + INTRADAY_RANGE + "）。今回取得できた期間 " + periodStart + " 〜 " + periodEnd + "（PR #88・#89 は 2026-06-26 〜 2026-09-24）。取れなかった日: " + (lostDays.length ? lostDays.join("・") : "なし") + "。" + PR88_INTRADAY_START + " の欠けは PR #90・#91 と同じ扱い（600本以上の条件を判定するときに、その日の足があれば増えていた本数を足す）。PR #88・#89 の窓にこの日が入っていた検証日は、8:50で " + shortWindowDays0850.length + "日、10:00で " + shortWindowDays1000.length + "日（窓が短いまま計算した銘柄日: 8:50 " + shortScored0850 + "件、10:00 " + shortScored1000 + "件）");
  ...
  L.push("- 時計: 8:50は対象日の 8:50、10:00は対象日の 10:00（日本時間）に固定（`currentSessionDate(\"JP\")` のずれ: 8:50 " + clockMismatch0850 + "日、10:00 " + clockMismatch1000 + "日。寄り付き後扱いになった8:50の件数 " + started0850 + "、寄り付き前扱いになった10:00の件数 " + notStarted1000 + "）");
  L.push("- 部品ごとの点数: `analyzeStock()` の返り値の `breakdown` から取り出した（部品の点数の合計とスコアの不一致 " + sumMismatch + "件）。`api/stock.js` の `toLocalDates()` の写しは実行時に一字一句同じであることを確かめた（" + (datesCopyOk ? "一致" : "不一致") + "）");
  L.push("");
  L.push("損益の定義（どちらも往復コスト0.1%を引いた値）:");
  L.push("");
  L.push("- 買値: 8:50は T当日の日足の始値（寄り付き）。10:00は10:00開始の15分足の始値");
  L.push("- 損益A（売り方ルール）: 利確ライン 買値 × 1.015・損切りライン 買値 × 0.9925。買った足から時刻順に15分足を見て、先に届いた方で決着。どちらにも届かなければ T当日の日足の終値で手仕舞い。同じ15分足で両方に届いた場合は PR #88 の按分（損切りが先 2/3・利確が先 1/3 の加重平均）");
  L.push("- 損益B（大引けまで持つ）: T当日の日足の終値 ÷ 買値 − 1 − 0.1%");
  L.push("- 勝率（損益A）: 損益Aがプラスだった割合。同じ足で両方に届いた件は 1/3 勝ちとして数える（PR #88 と同じ）");
  L.push("- 前日騰落率: 前日終値 ÷ 前々日終値 − 1（Yahoo の日足の調整後終値。`scripts/score-parts-check.mjs` と同じ）");
  L.push("- 当日騰落率: 10:00時点の現在値（9:45開始の15分足の終値）÷ 前日終値（Yahoo の日足の終値）− 1（`scripts/score-parts-1000-check.mjs` と同じ）");
  L.push("- グループ2B: 前日値上がり率上位" + CHANGE_TOP + "から入った候補");
  L.push("");
  ...
  L.push("| 項目 | 値 |");
  L.push("| --- | ---: |");
  L.push("| 検証日数 | " + testDays.length + "（" + rangeOf(testDays) + "） |");
  L.push("| 候補の銘柄日数 | " + candTotal + " |");
  L.push("| 8:50のスコアを計算できた銘柄日数（8:50の順位付けの対象） | " + result["0850"].all.length + " |");
  L.push("| └ うち8:50の買いまで判定できた銘柄日数 | " + traded(result["0850"].all, "0850").length + " |");
  L.push("| └ うち前日騰落率を計算できなかった銘柄日数 | " + result["0850"].riseMissing + " |");
  L.push("| 10:00のスコアを計算できた銘柄日数（10:00の順位付けの対象） | " + result["1000"].all.length + " |");
  L.push("| └ うち10:00の買いまで判定できた銘柄日数 | " + traded(result["1000"].all, "1000").length + " |");
  L.push("| └ うち当日騰落率を計算できなかった銘柄日数 | " + result["1000"].riseMissing + " |");
  ...
  L.push("| 8:50の売買から除いた銘柄日: T当日の日足の欠け・出来高0 | " + skipTrade["0850"].noDaily + " |");
  L.push("| 8:50の売買から除いた銘柄日: T当日の15分足が無い | " + skipTrade["0850"].noDayBars + " |");
  L.push("| 10:00の売買から除いた銘柄日: T当日の日足の欠け・出来高0 | " + skipTrade["1000"].noDaily + " |");
  L.push("| 10:00の売買から除いた銘柄日: 10:00開始の15分足が無い・始値が無い | " + skipTrade["1000"].noBuyBar + " |");
  L.push("| 10:00の売買から除いた銘柄日: 10:00以降の15分足が無い | " + skipTrade["1000"].noAfter + " |");
  L.push("");
  ...
  L.push("| 記号 | 作り方 |");
  L.push("| --- | --- |");
  L.push("| S0 | 今のスコア（`analyzeStock()` の返すスコアそのまま） |");
  L.push("| S1 | 部品の点数の合計から、下の「除く9部品」と「除く上限」を除いたもの（= 下の「S1 に残る部品」の点数の合計）。0〜100への切り詰めはしない |");
  L.push("| S2 | S1 − 除く9部品の点数の合計 |");
  L.push("| R | スコアを使わない。8:50は前日騰落率が低い順、10:00は当日騰落率が低い順 |");
  L.push("");
  L.push("- 除く9部品: " + EXCLUDED_PARTS.join("、"));
  L.push("- 除く上限: " + CAP_PARTS.join("、") + "（「上限抑制」には0〜100への切り詰めで動いた点を含む）");
  L.push("- S1 に残る部品: " + KEPT_PARTS.join("、"));
  L.push("- 上位10の選び方: 各日、その時点のスコアを計算できた候補を並べ、上から" + TOP_N + "銘柄。同点は候補の並び順（出来高上位" + NORMAL_VOL_TOP + "の順、続いてグループ2Bの順）が先のものを上にした（PR #88 と同じ）。R で騰落率を計算できなかった候補は最後に回した。11位以下は、その日の残りの候補");
  L.push("- 4通りとも、順位付けの対象と損益の計算は同じ。違うのは並べる値だけ");
  L.push("");

  // 3・4章 時点ごとの表
  SLOTS.forEach(function (sl, si) {
    var res = result[sl.key];
    L.push("## " + (3 + si) + ". " + sl.label + "時点（" + sl.buyNote + "）");
    L.push("");
    L.push("### " + (3 + si) + "-1. 上位10の成績");
    L.push("");
    L.push("| 選び方 | 件数 | 損益A 平均 | 損益B 平均 | 勝率（損益A） | グループ2Bの割合 | " + sl.riseLabel + "の平均 |");
    L.push("| --- | ---: | ---: | ---: | ---: | ---: | ---: |");
    var row1 = function (label, list) {
      var tr = traded(list, sl.key);
      return "| " + label + " | " + tr.length + " | " + pct3(avgAt(tr, sl.key, "retA")) + " | " + pct3(avgAt(tr, sl.key, "retB")) + " | " +
        pct1(avgAt(tr, sl.key, "winA")) + " | " + pct1(share2B(tr)) + " | " + pct2(avgAt(tr, sl.key, "rise")) + " |";
    };
    METHODS.forEach(function (me) { L.push(row1(me.label + " の上位10", res.byMethod[me.key].top)); });
    L.push(row1("候補全体", res.all));
    L.push("");
    L.push("- 件数は、上位10に入り、売買まで判定できた銘柄日の数（36日 × 10銘柄から、売買を判定できなかった銘柄日を除く）");
    L.push("- " + sl.riseLabel + "の平均は、同じ件数のうち" + sl.riseLabel + "を計算できた銘柄日の平均");
    L.push("");
    L.push("### " + (3 + si) + "-2. 上位10 − 11位以下（日ごとの差）");
    L.push("");
    L.push("期間: " + PERIODS.map(function (p) { return p.label + "（" + rangeOf(p.days) + "）"; }).join("、"));
    L.push("");
    L.push("| 選び方 | 期間 | 日数 | 損益A 差の平均 | 損益A t値 | 損益B 差の平均 | 損益B t値 |");
    L.push("| --- | --- | ---: | ---: | ---: | ---: | ---: |");
    METHODS.forEach(function (me) {
      PERIODS.forEach(function (p) {
        var inP = new Set(p.days.map(function (d) { return d.date; }));
        var pd = res.byMethod[me.key].perDay.filter(function (x) { return inP.has(x.date) && isNum(x.dA); });
        var a = tStat(pd.map(function (x) { return x.dA; })), b2 = tStat(pd.map(function (x) { return x.dB; }));
        L.push("| " + me.label + " | " + p.label + " | " + a.n + " | " + pct3(a.avg) + " | " + num2(a.t) + " | " + pct3(b2.avg) + " | " + num2(b2.t) + " |");
      });
    });
    L.push("");
    L.push("- 各日、上位10（売買まで判定できた銘柄日）の平均から11位以下（同）の平均を引き、その日ごとの差の平均と t値（平均 ÷（不偏標準偏差 ÷ √日数））を出した。どちらかの群が0件の日は除いた（PR #89 と同じ）");
    L.push("");
  });

  // 5章 突き合わせ
  L.push("## 5. S0 の上位10と PR #88・#89 のレポートの突き合わせ");
  L.push("");
  L.push("S0 の上位10の按分（損切りが先2/3）の成績を、PR #88・#89 のレポートの同じ表の同じ行と比べた。");
  L.push("");
  L.push("| 時点 | 比べた先 | 期待値（件数・平均） | 件数・平均の一致 | 今回（件数・平均損益率・勝率・利確・損切り・大引け） | レポートの行 | 行全体の一致 |");
  L.push("| --- | --- | --- | --- | --- | --- | --- |");
  CHECKS.forEach(function (c) {
    var oc = cellsOf(c.ours), tc = cellsOf(c.theirs);
    var sl = SLOTS.filter(function (x) { return x.key === c.slot; })[0];
    L.push("| " + sl.label + " | " + c.pr + "（`" + c.path.replace("../", "") + "` の「" + c.sec.replace("## ", "").replace(".", "") + "章」按分の「" + c.label + "」） | " + c.expectN + "件・" + c.expectAvg + " | " + (c.nAvgOk ? "一致" : "不一致") + " | " +
      oc.slice(1).join(" / ") + " | " + (tc ? tc.slice(1).join(" / ") : "見つからない") + " | " + (c.rowOk ? "一致" : "不一致") + " |");
  });
  L.push("");
  ...
  console.log("\n→ " + outPath);
  CHECKS.forEach(function (c) {
    if (!c.rowOk) console.log(c.pr + " と一致しない: " + c.ours + "\n  " + c.pr + ": " + c.theirs);
  });
};
