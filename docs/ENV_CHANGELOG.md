...
| 2026-08-28 | config | docs/ENV_CHANGELOG.md | 表の分断を修復し、内容列の書き方ルールと記入例を追加。PR #52 |  無 |
| 2026-08-30 | merge | tachibana-server | PR#19 寄り前ロガーの取得をチャンク分割・直列方式へ変更（立花APIの120件上限により121件目以降が黙って捨てられていた問題の根本対処）。環境変数 `PREMARKET_CHUNK_SIZE` を新設（既定80・起動時に一度だけ確定）。要求件数と応答件数の突き合わせ警告と、収集終了時の「件数不一致 Nティック」出力を追加。`tick失敗` の単位がティックからチャンクへ変更 | 有 |
...
| 2026-09-22 | merge | daytrade-simulator | 寄り予想機能の削除に伴い `PM_Q_SLOPE`・`PM_Q_INTERCEPT`・`TACHIBANA_MARKET_PRICE_API` のコード上の参照を削除。詳述セクション「2026-09-22 — 寄り予想機能の削除に伴う環境変数の参照削除」を参照 | 有 |
